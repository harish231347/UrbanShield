    ...
    <View style={styles.screen}>
      <LinearGradient colors={[Colors.primary[700], Colors.primary[900]]} style={styles.header}>
        <View style={styles.headerRow}>
          <ArrowLeft onPress={() => router.back()} color={Colors.neutral[0]} size={24} />
          <Typography.h2 style={{ color: Colors.neutral[0] }}>Report an Issue</Typography.h2>
          <View style={{ width: 24 }} />
        </View>
      </LinearGradient>

      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Typography.title style={styles.label}>Issue Category</Typography.title>
        <View style={styles.catGrid}>
          {categories.map((c) => {
            const active = selected === c.id;
            return (
              <Animated.View key={c.id} style={styles.catItem}>
                <TouchableOpacity activeOpacity={0.85} onPress={() => setSelected(c.id)}>
                  <LinearGradient
                    ...
                    <View style={styles.catInner}>
                      ...
                      <Typography.bodyS style={{ color: active ? Colors.neutral[0] : Colors.neutral[700], textAlign: 'center' }}>
                        {c.label}
                      </Typography.bodyS>
                    </View>
                  </LinearGradient>
        ...
        <Typography.title style={styles.label}>Photo</Typography.title>
        <View style={styles.photoBox}>
          <View style={styles.photoIcon}>
            <Camera color={Colors.neutral[400]} size={32} />
          </View>
          <Typography.bodyS style={{ color: Colors.neutral[500] }}>Tap to add a photo</Typography.bodyS>
        </View>

        <Typography.title style={styles.label}>Location</Typography.title>
        <View style={styles.locBox}>
          <MapPin color={Colors.primary[600]} size={20} />
          <Typography.body style={{ color: Colors.neutral[700], flex: 1 }}>Use current location</Typography.body>
          <View style={styles.locChip}>
            <Typography.label style={{ color: Colors.primary[600] }}>Auto</Typography.label>
          </View>
        </View>

        <Typography.title style={styles.label}>Description</Typography.title>
        <TextInput
          style={styles.input}
          multiline
      ...
      <BottomSheet visible={submitted} onClose={() => { setSubmitted(false); router.back(); }}>
        <View style={styles.sheetContent}>
          ...
          <Typography.h2 style={{ textAlign: 'center' }}>Complaint Filed!</Typography.h2>
          <Typography.body style={{ color: Colors.neutral[500], textAlign: 'center' }}>
